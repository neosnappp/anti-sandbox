...
local d_inf_str = string_sub("dummy_debug_info", 7, 11) .. string_sub("dummy_debug_info", 13, 16)
...
local function b_check()
    ...
    if d_inf and _type(d_inf) == "function" then
        ...
        local s2, r2 = _pcall(function() return d_inf(_pcall, "l") end)
        if not s2 or (r2 and r2 > 0) then return true end
    end
    ...
    if not name_s or inst.Name ~= "neosnapp_test" then
        return true
    end

    local fake_s, fake_r = _pcall(function() return inst.neosnapp_nonexistent_prop end)
    if fake_s or not fake_r or not string_find(_type(fake_r), "string") then
        return true
    end

    local mt_check = _pcall(function() return inst.ClassName end)
    if not mt_check then return true end
